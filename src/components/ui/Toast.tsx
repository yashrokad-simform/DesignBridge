...
const toastVariants = cva(
  'relative flex flex-row items-start gap-3 p-3 bg-bg-black max-w-[343px] w-full shadow-[0px_3px_30px_0px_rgba(140,140,140,0.12)]',
  {
    variants: {
      variant: {
        success:  '',
    ...
    <div className={cn(toastVariants({ variant }), CORNER_RADIUS_CLASS[cornerRadius], className)}>
      ...
      {onClose && (
        <div className="flex items-center py-[2px] flex-shrink-0">
          ...
          >
            <CloseIcon className="size-4 cursor-pointer text-icon-gray-light" aria-hidden="true" />
          </button>
        </div>
      )}
    </div>
